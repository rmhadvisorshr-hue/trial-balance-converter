...
const TAX_RATE = 0.3; // LLPs are taxed exactly like partnership firms - flat rate, not individual slabs
const CESS_RATE = 0.04;

// 'STAT': computation of total income and tax payable, identical statutory
// treatment to Partnership's STAT (an LLP is taxed as a firm under the
// Income Tax Act, not under the individual Section 115BAC slabs used for a
// Proprietor). Net Profit as per the Appropriation A/c, the flat 30%+4%-
// cess rate, and TDS (from any TDS-named OTHER_CURR_ASSETS ledger) are all
// derivable from the trial balance; disallowed expenses, unabsorbed losses,
// Chapter VIA deductions, advance tax and self-assessment tax are external
// facts a TB can't supply, same "manual/0" pattern used throughout this
// app. Every cached value below is computed step-by-step in the same order
// as the formula chain, so the displayed figures match what each formula
// actually evaluates to on recalculation.
