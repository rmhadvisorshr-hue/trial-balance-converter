...
export interface FixedAssetScheduleResult {
  closingTotalRow: number; // 'FIRM FA' Total row, column G - for BS
  depForYearRow: number; // same Total row, column F (depreciation) - for P&L
}

// 'FIRM FA': asset-by-asset, same approach as the Pvt Ltd '11. FA' /
// Partnership 'FIRM FA' register - a trial balance only gives the closing
// net book value per asset ledger, never the WDV rate, opening balance, or
// pre/post-3-Oct addition split (the reference file's own rates/openings
// are typed-in figures, not TB-derived even there - and it also carries
// land/shop-premises entries at a 0% rate, which this app can't distinguish
// from depreciable assets by name alone). Rate/Opening/Additions default to
// 0 (manual); Closing is the known TB value per asset, and one
// "unallocated" row carries the real total depreciation from the TB's P&L
// Depreciation ledger, so the schedule's totals still tie to real figures.
