...
export interface TradingPLResult {
  netProfitRow: number; // 'P&L' sheet row holding "To/By Net Profit c/d" - for FIRM CAP&BS
}
...
// so blank cells on the shorter side just contribute 0. Same helper as
// partnershipFull/tradingPL.ts (kept file-local rather than shared, since
// each entity's P&L sheet has its own header block).
...
// 'P&L(V)' is the vertical restatement of the same figures - reuses the
